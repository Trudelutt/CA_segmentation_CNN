...
def precision(y_true, y_pred):
    ...
    true_positives = K.sum(K.round(K.clip(flatten_true * flatten_pred, 0, 1)))
    predicted_positives = K.sum(K.round(K.clip(flatten_pred, 0, 1)))
...
def recall(y_true, y_pred):
    ...
    flatten_true = K.flatten(y_true)
    true_positives = K.sum(K.round(K.clip(flatten_true * flatten_pred, 0, 1)))
    possible_positives = K.sum(K.round(K.clip(flatten_true, 0, 1)))
